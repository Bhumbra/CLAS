...
//---------------------------------------------------------------------------
void cblas_outer(double* Out, double* In0, double* In1, 
...
{
	std::cout << "-----------------------------------------------------" << std::endl;
	std::cout << "CBLAS vs CLAS test program benchmarking outer product" << std::endl << std::endl;
	...
	if (argc < 5) {
		...
		std::cout << "k = second dimension multiplicand" << endl;
	...
	double* b = new double[m * p * q];
	double* c = new double[m * p * q * k]; 
	...
	tic(t0);
	for (i = 0; i<N; i++) {
		cblas_outer(c, a, b, m, k, p, q);	
	}
	dt = 1000.*toc(t0);
	std::cout << "cblas_dger:" << std::endl;
	std::cout << "\t Total time = " << dt << " ms" << std::endl;
	std::cout << "\t Mean time per rep. = " << dt/(double)(N) << " ms" << std::endl << std::endl;

	tic(t0);
	for (i = 0; i<N; i++) {
		clas::outer_product_double(c, a, b, m, k, p, q, false, false, (double*)0, 0, 0.);
	}
	dt = 1000.*toc(t0);
	std::cout << "clas::outer_product_double (single-thread):" << std::endl;
	std::cout << "\t Total time = " << dt << " ms" << std::endl;
	std::cout << "\t Mean time per rep. = " << dt/(double)(N) << " ms" << std::endl << std::endl;

	tic(t0);
	for (i = 0; i<N; i++) {
		clas::outer_product_double(c, a, b, m, k, p, q);
	}
	dt = 1000.*toc(t0);
	std::cout << "clas::outer_product_double (multithreaded):" << std::endl;
	std::cout << "\t Total time = " << dt << " ms" << std::endl;
	std::cout << "\t Mean time per rep. = " << dt/(double)(N) << " ms" << std::endl << std::endl;
...
void cblas_outer(double* Out, double* In0, double* In1, 
	...
	if (!I0T) {
		...
		for (i = 0; i < p; i++) {
			...
			in1 = In1 + i * In1S;
			for (j = 0; j < m; j++) {
				cblas_dger (CblasRowMajor, k, q, 1.,
									 	in0, 1, in1, 1, out + j * Outs, q);
				in0 += In0s;
				in1 += In1s;
			}
		}
